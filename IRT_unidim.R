...
IRT_unidim <- function() {
  ...
  process_data <- function() {
    ...
    # Executar análise IRT com as colunas corretas de acordo com a área
    result <- run_irt_analysis(dataset_filtered, model, tx_respostas_col, tx_gabarito_col, area_code)
  ...
  run_irt_analysis <- function(data, model_type, tx_respostas_col, tx_gabarito_col, area_code) {
    # Obter o código de prova do escopo pai (process_data)
    prova_code <- as.integer(tclvalue(selected_prova))
    start_time <- Sys.time()
    cat("[INFO] Iniciando análise IRT unidimensional...\n")
    ...
    # Preparar matriz de respostas com as colunas com sufixo da área
    response_matrix <- prepare_response_matrix(data[[tx_respostas_col]], data[[tx_gabarito_col]])
    
    # Remover linhas que contêm apenas NAs
    rows_to_keep <- apply(response_matrix, 1, function(row) sum(!is.na(row)) > 0)
    if (sum(!rows_to_keep) > 0) {
      cat(sprintf("[INFO] Removendo %d linhas que contêm apenas NAs\n", sum(!rows_to_keep)))
      response_matrix <- response_matrix[rows_to_keep, ]
    }
    
    # Verificar dimensões da matriz
    cat(sprintf("[INFO] Matriz de respostas após limpeza: %d alunos x %d itens\n", nrow(response_matrix), ncol(response_matrix)))
    
    # Definir nomes únicos para as colunas (itens)
    colnames(response_matrix) <- paste0(area_code, "_item_", sprintf("%02d", seq_len(ncol(response_matrix))))
    ...
    cat(sprintf("[INFO] Estimando modelo unidimensional: %s\n", model_spec))
    
    # Estimar modelo IRT com os parâmetros escolhidos
    cat(sprintf("[INFO] Estimando modelo %s...\n", model_spec))
    
    # Definir parâmetros com base na documentação oficial do mirt
    technical_opts <- list()
    technical_opts$NCYCLES <- max_iter
    
    # Criar configuração para o modelo
    config <- list(
    ...
    # Definir a faixa de theta
    if (theta_min != -4 || theta_max != 4) {
      config$customTheta <- as.matrix(seq(theta_min, theta_max, length.out = quadpts))
    }
    
    # Configurar modelo para unidimensional ou preparação para MIRT
    model_dimension <- 1  # Unidimensional por padrão
    
    # Prever futura expansão para MIRT (Multidimensional IRT)
    # Mantemos o modelo unidimensional aqui, mas com estrutura para futuras implementações:
    # 1. Modelos com correlações entre dimensões
    # 2. Modelos bifator (fator geral + fatores específicos)
    # 3. Modelos com itens em múltiplas dimensões
    
    # Otimizar uso de CPU usando o sistema de paralelismo em dois níveis
    if (num_cores > 1) {
    ...
    # Estimar o modelo com as configurações
    model <- mirt(
      response_matrix, 
    ...
    cat(sprintf("[INFO] Velocidade: %.2f itens/segundo\n", ncol(response_matrix) / as.numeric(estimation_time)))
    ...
    result <- data.frame(
      domain = character(ncol(response_matrix)),
      item = character(ncol(response_matrix)),
      model = model_type,
      a = numeric(ncol(response_matrix)),
      b = numeric(ncol(response_matrix)),
      c = numeric(ncol(response_matrix)),
      gabarito = character(ncol(response_matrix)),
      students = rep(nrow(response_matrix), ncol(response_matrix))
    )
    
    # Extrair parâmetros e gabaritos
    for (i in seq_len(ncol(response_matrix))) {
      # Usar a área correta como domínio
      domain_code <- area_code
      item_num <- sprintf("%02d", i)
    ...
    cat(sprintf("Estimados parâmetros para %d itens usando modelo %s\n\n", ncol(response_matrix), model_type))
  ...
  compare_with_inep <- function(result, prova_code, area_code) {
    tryCatch({
      ...
      if (nrow(comp_table) > 0) {
        ...
        cat(sprintf("\n[TOP 5 QUESTÕES COM MAIOR ERRO]\n"))
        # Top 5 combinados (erro máximo entre a, b, c)
